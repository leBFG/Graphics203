...
namespace
{
    ...
    //E - D3D12MA modified version
    //--------------------------------------------------------------------------------------
    HRESULT CreateTextureResource(
        _In_ D3D12MA::Allocator* d3dAllocator,
        D3D12_RESOURCE_DIMENSION resDim,
        size_t width,
        size_t height,
        size_t depth,
    ...
    //    const CD3DX12_HEAP_PROPERTIES defaultHeapProperties(D3D12_HEAP_TYPE_DEFAULT);
        ...
        hr = d3dAllocator->CreateResource3(
            &allocDesc,
            &desc,
    ...
    //--------------------------------------------------------------------------------------
    HRESULT CreateTextureFromDDS(_In_ ID3D12Device* device ,_In_ D3D12MA::Allocator* allocator,
        _In_ const DDS_HEADER* header,
        _In_reads_bytes_(bitSize) const uint8_t* bitData,
        size_t bitSize,
        size_t maxsize,
        ...
        _Out_opt_ bool* outIsCubeMap) noexcept(false)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            hr = CreateTextureResource(allocator, resDim, twidth, theight, tdepth, reservedMips - skipMip, arraySize,
                format, resFlags, loadFlags, texture);
    ...
    bool* isCubeMap)
{
    ...
    if (!d3dAllocator || !fileName || !allocation)
    {
        return E_INVALIDARG;
    }
    ...
    std::vector<D3D12_SUBRESOURCE_DATA> subresources;
    hr = CreateTextureFromDDS(d3ddevice, d3dAllocator,
        header, bitData, bitSize, maxsize,
        resFlags, loadFlags,
        allocation, subresources, isCubeMap);
    ...
    /*    resourceUpload.Transition(
        ...
        CD3DX12_TEXTURE_BARRIER textureBar(D3D12_BARRIER_SYNC_COPY, D3D12_BARRIER_SYNC_ALL_SHADING, D3D12_BARRIER_ACCESS_COPY_DEST, D3D12_BARRIER_ACCESS_SHADER_RESOURCE, D3D12_BARRIER_LAYOUT_COPY_DEST, D3D12_BARRIER_LAYOUT_SHADER_RESOURCE, (*allocation)->GetResource(),CD3DX12_BARRIER_SUBRESOURCE_RANGE(0xFFFFFFFF));
        CD3DX12_BARRIER_GROUP group(1,&textureBar);
        
        resourceUpload.Barrier(1, &group);

        // If it's missing mips, let's generate them
#if defined(_MSC_VER) || !defined(_WIN32)
        const size_t mipLevels = (*allocation)->GetResource()->GetDesc().MipLevels;
#else
        D3D12_RESOURCE_DESC tmpDesc;
        const size_t mipLevels = (*texture)->GetDesc(&tmpDesc)->MipLevels;
#endif
        ...
        if ((loadFlags & DDS_LOADER_MIP_AUTOGEN) && subresources.size() != mipLevels)
        {
            resourceUpload.GenerateMips((*allocation)->GetResource());
        }
    }
