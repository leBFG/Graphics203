...
#else
#include "DXIL/GenerateMips_main.inc"
#endif
...
public:
    ...
    void Barrier(uint32_t groupnum, const D3D12_BARRIER_GROUP* groups)
    {
        mList.Get()->Barrier(groupnum, groups);
    }
...
private:
    ...
    ComPtr<ID3D12GraphicsCommandList10>          mList;
...
void __cdecl DirectX::ResourceUploadBatch::Barrier(uint32_t groupnum, D3D12_BARRIER_GROUP* groups)
{
    pImpl->Barrier(groupnum, groups);
}
