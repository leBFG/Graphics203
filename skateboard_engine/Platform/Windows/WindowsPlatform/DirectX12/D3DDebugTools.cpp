...
void D3DDebugTools::D3DMessageHandler(D3D12_MESSAGE_CATEGORY Category, D3D12_MESSAGE_SEVERITY Severity, D3D12_MESSAGE_ID ID, LPCSTR pDescription, void* pContext)
{
	...
	case D3D12_MESSAGE_SEVERITY_CORRUPTION:
		SKTBD_MSG_CRITICAL(pDescription); break;
	case D3D12_MESSAGE_SEVERITY_ERROR:
		SKTBD_MSG_ERROR(pDescription); break;
	case D3D12_MESSAGE_SEVERITY_WARNING:
		SKTBD_MSG_WARN(pDescription); break;
	case D3D12_MESSAGE_SEVERITY_INFO:
		SKTBD_MSG_INFO(pDescription); break;
	case D3D12_MESSAGE_SEVERITY_MESSAGE:
	default:
		SKTBD_MSG_TRACE(pDescription); break;
	}
	...
	if (gD3DContext && gD3DContext->CheckDeviceRemovedStatus())
	{
		...
		D3D_CHECK_FAILURE(gD3DContext->GetDevice()->QueryInterface(IID_PPV_ARGS(&pDred)));
		...
		{
			SKTBD_MSG_INFO("No auto-breadcrumbs data was retrieved from DRED.");
		}
		...
		{
			SKTBD_MSG_INFO("No page fault data was retrieved from DRED");
		}
	}
...
void D3DDebugTools::PIXGPUCaptureFrame(UINT frameCount)
{
	...
	if (SUCCEEDED(result))
	{
		SKTBD_MSG_INFO(L"Capture saved as: {}", captureName.c_str());
	}
	else
	{
		SKTBD_MSG_ERROR("Capture failed.");
	}
}
...
{
	SKTBD_MSG_INFO("DRED Auto Breadcrumbs:");
	SKTBD_MSG_INFO("-------------");
	...
	while (breadcrumb)
	{
		...
		if (breadcrumb->BreadcrumbCount && breadcrumb->BreadcrumbCount > lastBreadcrumb)
		{
			...
			{
				SKTBD_MSG_INFO(L"Command Queue: {}", breadcrumb->pCommandQueueDebugNameW);
			}
			if (breadcrumb->pCommandListDebugNameW)
			{
				SKTBD_MSG_INFO(L"Command List: {}", breadcrumb->pCommandListDebugNameW);
			}

			if (breadcrumb->pCommandHistory)
			{
				SKTBD_MSG_INFO("Operation History:");
				auto command = breadcrumb->pCommandHistory;
				for (UINT i = 0; i < breadcrumb->BreadcrumbCount && command; i++)
				{
					if (i == lastBreadcrumb)
					{
						SKTBD_MSG_WARN("--->{}", BreadCrumbOpToStr(*command));
					}
					else
					{
						SKTBD_MSG_INFO("    {}", BreadCrumbOpToStr(*command));
					}
					command++;
				}
			}
...
{
	SKTBD_MSG_INFO("Page Fault VA: {}", pageFault->PageFaultVA);

	auto allocation = pageFault->pHeadExistingAllocationNode;
	if (allocation)
	{
		SKTBD_MSG_INFO("Existing objects with matching VA:");
		while (allocation)
		{
			SKTBD_MSG_INFO(L"	Name: {}", allocation->ObjectNameW);
			SKTBD_MSG_INFO("	Type: {}", AllocationTypeToStr(allocation->AllocationType));
			SKTBD_MSG_INFO("    -------------");
	...
	{
		SKTBD_MSG_INFO("No existing objects matching VA.");
	}

	allocation = pageFault->pHeadRecentFreedAllocationNode;
	if (allocation)
	{
		SKTBD_MSG_INFO("Recently freed objects with matching VA:");
		while (allocation)
		{
			SKTBD_MSG_INFO(L"	Name: {}", allocation->ObjectNameW);
			SKTBD_MSG_INFO("	Type: {}", AllocationTypeToStr(allocation->AllocationType));
	...
	{
		SKTBD_MSG_INFO("No recently freed objects matching VA.");
	}
}
